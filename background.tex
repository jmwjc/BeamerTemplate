...
\begin{columns}
    \column{0.45\textwidth}
        ...
        \begin{align*}
            ...
            \kappa_{\alpha\beta} &= (\Gamma^\gamma_{\alpha\beta}\boldsymbol u_{,\gamma} - \alert{\boldsymbol u_{,\alpha\beta}}) \cdot \boldsymbol a_3 = - \boldsymbol u_{,\alpha\vert\beta} \cdot \boldsymbol a_3
        \end{align*}
...
\begin{columns}
    \column{0.5\textwidth}
        ...
        \int_\Omega h \delta \varepsilon_{\alpha\beta} C^{\alpha\beta\gamma\eta} \varepsilon_{\gamma\eta}(\boldsymbol p)d\Omega
        +\int_\Omega \frac{h^3}{12}\delta \kappa_{\alpha\beta}C^{\alpha\beta\gamma\eta}\kappa_{\gamma\eta}(\boldsymbol p)d\Omega = f(\boldsymbol p)
        \end{equation*}}
        }
...
\begin{tcolorbox}[colframe=MediumBlue,title=伽辽金无网格法误差估计 {\footnotesize(Wu and Wang, 2021)},height=3.4cm,valign=center]
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \draw[-{Stealth}, thick, red](a.south) -- ++(0,-0.3) node[below]{\bfseries 一致性条件控制};
        \draw[-{Stealth}, thick, red](b.north) -- ++(0,0.3) node[above]{\bfseries 变分一致性条件控制};
    \end{tikzpicture}
